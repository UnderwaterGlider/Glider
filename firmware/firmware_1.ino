...
//Location we want the data to be put. (global variable)
int eeAddressPiston = 0; // Piston
int eeAddressMass = eeAddressPiston + sizeof(int); // Mass
int maxTurn1 = 0;
int maxTurn2 = 0;
...
void putEE(int eeAddress, int CalValue)
{
EEPROM.put(eeAddress, CalValue);
}

int getEE(int eeAddress)
{
int MaxTurn;
EEPROM.get(eeAddress, MaxTurn);
return MaxTurn;
}
...
digitalWrite(StepPin, switcher); // First - low, following - high - /is it normal?/ In first interrupt sD = 1, and it's normal. When we have even number, last bit
in sD ever is 0
this->incStepEngine();
}
else
...
void loop()
{
maxTurn1 = piston.getEE(eeAddressPiston);
maxTurn2 = mass.getEE(eeAddressMass);
piston.movee(200, 2);
mass.movee(-200, 1);
delay(2000);
piston.movee(-200, 1);
